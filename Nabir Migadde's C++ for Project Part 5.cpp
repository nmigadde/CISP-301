...
Verison: 1.5
~Part 5 reading and writing to text/data files from storage
Revised by: Nabir Migadde
Revision date: July 26, 2017
*/
...
int main() {
	...
	ofstream outfile; //Writing to file "results"
	outfile.open(fileName, ios::app); //Adding to exisiting information
	...
	while (computeAns == 1) {
		...
		outfile << agentID[j-1] << "  " << modelType[j-1] << "  " << housePrice[j-1] << " " << commissions[j-1] << endl; //Variables to be added to file
	...
	outfile.close(); //Close or Stop writing to file
	...
	if (housePrice[0] > 0.0 && housePrice[0] < 500000000.0) {
		...
		cout << "   All Results" << endl << endl;
		cout << "  ID   Model Price  Comission" << endl;

		ifstream infile; //Reading from file "results"
		infile.open(fileName);

		while (!infile.eof()) {
			infile >> ID >> model >> price >> commission; 
			cout << " " << ID << "   " << model << "   " << price << "     " << commission << endl; //Display information in file to user
		}

		infile.close(); //Close or Stop reading from file "results"
